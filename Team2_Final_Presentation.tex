...
\begin{frame}{Problem Statement}
  ...
  \begin{itemize}
    ...
    \item \textbf{Collect additional data from modern forums}, evaluate and interpret the key findings
    \item \textbf{Add a way for the model to generate a confidence score} indicating how certain it is about its decision
...
\begin{frame}{OOD Detection -- Results \& The FP\,/\,FN Trade-off}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{Temperature Comparison}
      ...
      \begin{tabular}{@{}c c c c@{}}
        \toprule
        \textbf{Temp $T$} & \textbf{AUROC} & \textbf{AP} & \textbf{FPR@TPR70} \\
        \midrule
      ...
      \normalsize
      Best at $T = 10$:\; AUROC\,=\,0.8026,\; AP\,=\,0.8893
    \end{column}
...
\begin{frame}{Pipeline Summary}
  \begin{enumerate}
    \item \textbf{Data Loading:} 20 Newsgroups via HuggingFace $\rightarrow$ Train/Val/Test split
    \vspace{0.15cm}
    \item \textbf{Tokenisation:} ModernBERT tokenizer, max\_len=256
    \vspace{0.15cm}
    \item \textbf{Model Setup:} ModernBERT-Large, 50\% layers frozen
    \vspace{0.15cm}
    \item \textbf{Hyperparameter Optimisation:} Optuna QRS (LR, weight decay, warmup ratio)
    \vspace{0.15cm}
    \item \textbf{Model Selection:} Top~3 $\rightarrow$ evaluate on test set
    \vspace{0.15cm}
    \item \textbf{OOD Detection:} MSP + Temp. Scaling: ID=20 Newsgroups, OOD=AG News
    \vspace{0.15cm}
    \item \textbf{Real-World Evaluation:} Evaluate on collected Reddit data, compare \& interpret vs test results
    \vspace{0.15cm}
    \item \textbf{Uncertainty Estimation:} Confidence score \& certainty of the model (MSP, TTA)
  \end{enumerate}
\end{frame}
